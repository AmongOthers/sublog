...
(function () {
   ...
   * elements.
   **/
  function highlight(text, tabReplace, useCodeBlocks, showLn){
    var i, p = /class=['"]\s*(\S*)\s*(\S*).*['"]/, classes = null, currentClass, lang = null, hljsLanauages = hljs.LANGUAGES, showlnVal = null, result = null;
    ...
    if (!!useCodeBlocks) {
      ...
      return text
        .replace(/\n/g,'\uffff')
        .replace(/<pre>\s*<code([^>]*)>(.*?)<\/code>\s*<\/pre>/gm, function(original, attrs, source){
          if(attrs.match(/no-highlight/)) {
            return original;
          }
          lang = null;
          classes = attrs.match(p);
          if(classes) {
            for(i = 1; i < classes.length; i++) {
              currentClass = classes[i];
              if(currentClass === "ln_on") {
                showLn = true;
              } else if(currentClass === "ln_off") {
                showLn = false;
              } else if(lang === null) {
                lang = currentClass;
              }
          ...
          var sourceText = source.replace(/\uffff/g,"\n");
          if(showLn) {
            return lnHighlightText(sourceText, tabReplace, attrs, lang);
          } else {
            result = hljs.highlightText(sourceText, tabReplace, false, lang);
            return '<pre><code' + attrs + ' data-sublog-lang=\"' + result.language + '\">'+ result.value + '</code></pre>';
          }
        })
        .replace(/&amp;(\w+;)/g,'&$1').replace(/\uffff/g,"\n");
    } else {
        return hljs.highlightText(source.replace(/\uffff/g,"\n"), tabReplace, attrs).value;
    }
  }

  function lnHighlightText(text, tabReplace, attrs, lang) {
	  // var lines = text.split('\n'), len = lines.length - 1, indexLines = [],
	  //   maxValueStrLen = (len.toString()).length, lenMinus, i, j, spaces, indexs, result, html;
	  // for(i = 0; i < len; i++) {
  	// 	lenMinus = maxValueStrLen - i.toString().length; 
  	// 	spaces = "";
  	// 	for(j = 0; j < lenMinus; j++) {
  	// 		spaces = " " + spaces;
  	// 	}
	  // 	indexLines.push(spaces + i);
	  // }
	  // indexs = indexLines.join('\n');
   //  result = hljs.highlightText(text, tabReplace, false, lang);
   //  html =  "<div class=\"code\"><pre style=\"float: left\"><code>" + indexs + "</code></pre>" + 
   //    "<pre><code" + attrs + " data-sublog-lang=\"" + result.language + "\">" + result.value + "</code></pre></div>";

    var i, after = [], result, lines, html, len;
    result = hljs.highlightText(text, tabReplace, false, lang);
    lines = result.value.split("\n");
    len = lines.length - 1;
    for(i = 0; i < len; i++) {
      lines[i] = ("<li><code>" + lines[i] + "</code></li>");
    }
    html = '<pre><ol' + attrs + ' data-sublog-lang=\"' + result.language + '\">'+ lines.join("") + '</ol></pre>';
    return html;
  }
  ...
  function backwardsCompat() {
    ...
    var lang, langs = [];
    for(lang in availableMap) {
      langs.push(lang);
    }
    Highlight.init(function () {}, langs);
    //Highlight.init(function () {}, ['php', 'javascript']);
