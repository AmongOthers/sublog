...
(function () {
      ...
      return text
        ...
        .replace(/<pre>\s*<code([^>]*)>(.*?)<\/code>\s*<\/pre>/gm, function(original, attrs, source){
          if(showLn) {
            ...
            return '<pre><code'+attrs+'>'+hljs.highlightText(source.replace(/\uffff/g,"\n"), tabReplace)+'</code></pre>';
          }
        })
  ...
  function lnHightlightText(text, tabReplace, attrs) {
	  var lines = text.split('\n'), len = lines.length - 1, indexLines = [],
	    maxValueStrLen = (len.toString()).length, lenMinus, i, j, spaces, indexs;
	  for(i = 0; i < len; i++) {
      ...
      return "<div class=\"code\"><pre style=\"float: left\"><code>" + indexs + "</code" + attrs + " ></pre><pre><code>" + hljs.highlightText(text, tabReplace) + "</code></pre></div>";
  }
