...
(function () {
  ...
  function highlight(text, tabReplace, useCodeBlocks, showLn){
    var i, p = /class="(\S*)\s*(\S*)"/, classes, lang, hljsLanauages = hljs.LANGUAGES, result;
    ...
    if (!!useCodeBlocks) {
      ...
      return text
        ...
        .replace(/<pre>\s*<code([^>]*)>(.*?)<\/code>\s*<\/pre>/gm, function(original, attrs, source){
          ...
          if(!hljsLanauages.hasOwnProperty(lang)) {
          ...
          if(showLn) {
            return lnHightlightText(source.replace(/\uffff/g,"\n"), tabReplace, attrs, lang);
          } else {
            result = hljs.highlightText(source.replace(/\uffff/g,"\n"), tabReplace, false, lang);
  ...
  function lnHightlightText(text, tabReplace, attrs, lang) {
	  var lines = text.split('\n'), len = lines.length - 1, indexLines = [],
	    maxValueStrLen = (len.toString()).length, lenMinus, i, j, spaces, indexs, result;
	  for(i = 0; i < len; i++) {
    ...
    return "<div class=\"code\"><pre style=\"float: left\"><code>" + indexs + "</code></pre>" + 
